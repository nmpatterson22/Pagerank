...
class WebGraph():
    ...
    def make_personalization_vector(self, query=None):
        ...
        if query is None:
            ...
            #creating for loop to go through URL's and then create conditional 0 entries 
            for i in range(n): 
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            # row_sum is a looping function
            a = torch.zeros(n)
            row_sum = torch.sparse.sum(self.P,1)
            ...
            #Create intial x 
            x = x0
            #Defining Equation 5.1 (left hand side) and creating loop
            for k in range (0, max_iterations):
                x1 = x
                x_transpose_alpha = x1.t() * alpha

                left_hand_side = alpha * torch.sparse.mm(self.P.t(),x1).t()


                #Defining Equation 5.1 (right hand side)
                right_hand_side = (a * x_transpose_alpha + (1-alpha))*v.t()


                #Create resulting vector by combining left and right hand side of equation
                x = (left_hand_side + right_hand_side).t()

                #Creating break to stop iterating over function when difference between vectors less than epsilon
